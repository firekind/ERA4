...
class App:
        ...
        while dpg.is_dearpygui_running():
            match self._sim_phase:
                ...
                case SimSetupPhase():
                    ...
                    self._dashboard.update_sim_view(frame)
                case SimActivePhase():
                    res = self._sim_phase.update()
                    if res is not None:
                        stats, frame = res
                        if frame is not None:
                            self._dashboard.update_sim_view(frame)
                        self._dashboard.update_stats_view(stats)
    ...
    def _transition_to_active_phase(self) -> bool:
        match self._sim_phase:
            case SimSetupPhase():
                start_point = self._sim_phase.start_point
                waypoints = self._sim_phase.waypoints
                map = self._sim_phase.map
                config = self._sim_phase.config
                ...
                try:
                    self._sim_phase = SimActivePhase(
                        map=map,
                        config=config,
            ...
            case SimActivePhase():
                self._sim_phase.resume_simulation()
                return True
    ...
    def _on_sim_pause(self):
        match self._sim_phase:
            case SimActivePhase():
                self._sim_phase.pause_simulation()

    def _on_sim_view_right_click(self, mouse_pos: tuple[int, int]):
        match self._sim_phase:
            case SimSetupPhase():
                self._sim_phase.show_menu(mouse_pos)
...
class SimActivePhase:
    ...
    def update(self) -> tuple[TrainerStats, NDArray[np.uint8] | None] | None:
        if self._running:
...
class ControlPanel:
    ...
    def reset(self):
        if self._start_button is not None:
            dpg.set_item_label(self._start_button, "Start / Resume")
        self._is_sim_running = False
...
class StatsPanel:
    ...
    def update(self, stats: TrainerStats):
        if self._step_item_id is not None:
            dpg.set_value(self._step_item_id, f"{stats.step}")

        if self._episode_item_id is not None:
            dpg.set_value(self._episode_item_id, f"{stats.episode}")

        if self._reward_item_id is not None:
            dpg.set_value(self._reward_item_id, f"{stats.reward:.04f}")

        if self._temperature_item_id is not None:
            dpg.set_value(self._temperature_item_id, f"{stats.temperature:.04f}")
        ...
        if self._ema_reward_item_id is not None:
            dpg.set_value(self._ema_reward_item_id, f"{ema:.04f}")

        self._update_reward_graph()

    def reset(self):
        if self._step_item_id is not None:
            dpg.set_value(self._step_item_id, "0")

        if self._episode_item_id is not None:
            dpg.set_value(self._episode_item_id, "0")

        if self._reward_item_id is not None:
            dpg.set_value(self._reward_item_id, "0")

        if self._ema_reward_item_id is not None:
            dpg.set_value(self._ema_reward_item_id, "0")

        if self._temperature_item_id is not None:
            dpg.set_value(self._temperature_item_id, "0")
    ...
    def _update_reward_graph(self):
        if (
            self._reward_history_series is None
            or self._reward_history_x_axis is None
            or self._reward_history_y_axis is None
            or self._ema_reward_history_series is None
            or self._ema_reward_history_x_axis is None
            or self._ema_reward_history_y_axis is None
        ):
            return
        ...
        dpg.fit_axis_data(self._reward_history_x_axis)
        dpg.fit_axis_data(self._reward_history_y_axis)
        ...
        dpg.fit_axis_data(self._ema_reward_history_x_axis)
        dpg.fit_axis_data(self._ema_reward_history_y_axis)
